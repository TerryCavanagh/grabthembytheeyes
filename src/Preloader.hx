...
class Preloader extends NMEPreloader{
	override public function new(){
		...
		backcol = RGB(128, 128, 128);
		frontcol = RGB(192, 192, 192);
		loadercol = RGB(192, 192, 192); 
		loadingbarwidth = 250;
		//----------------
	...
	override public function onUpdate(bytesLoaded:Int, bytesTotal:Int){
		...
		fillrect(Std.int((screenwidth/2) - (loadingbarwidth/2) - 2)-1, Std.int((screenheight / 2) - 23), loadingbarwidth+4+2, 42, frontcol);
		fillrect(Std.int((screenwidth / 2) - (loadingbarwidth / 2))-1, Std.int((screenheight / 2) - 21), loadingbarwidth+2, 38, backcol);
		for(i in 0...Std.int(25*p)){
		  fillrect(Std.int((screenwidth / 2) - (loadingbarwidth / 2)) + 1 + (i*10), Std.int((screenheight / 2) - 20), 8, 38 - 2, loadercol);
		}
