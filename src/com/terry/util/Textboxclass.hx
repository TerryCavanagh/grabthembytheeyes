...
class Textboxclass{
	...
	public function centerx():Void { resize();	xp = Std.int((Def.SCREENWIDTH / 2) - (width / 2));	resize(); }
	public function centery():Void { resize();	yp = Std.int((Def.SCREENHEIGHT / 2) - (height / 2)); resize(); }
	
	public function adjust():Void {
		resize();
		if (xp < 3) xp = 3;
		if (yp < 3) yp = 3;
		if (xp + width > Def.SCREENWIDTH - 3) xp = (Def.SCREENWIDTH - 3) - width;
		if (yp + height > Def.SCREENHEIGHT - 3) yp = (Def.SCREENHEIGHT - 3) - height;	
		resize();
	}
	
	public function update():Void {
		if (textboxstate == Textbox.STATE_BOXAPPEARING) {
			lerp += .1;	if (lerp >= 1) { lerp = 1; }
			if (lerp >= 1) {
	...
	public function resize():Void {
		...
		//old resize function
		max = 0;
		for (iter in 0...numlines) if (Text.len(line[iter]) > max) max = Std.int(Text.len(line[iter]));
